...
public class AnonotationTest {
    public static void main(String[] args) throws Exception{
        ...
        student1.setId("101");
        student1.setName("张三jjj");
        student1.setSex(15);
        student1.setBirthday(new Date());
        student1.setRegistrationDate(new Date());

        String billNo = "CGRKD-20221212-2444";
        String fileUrl = "D:\\self_project\\practice\\practice-gz\\src\\main\\java\\basic\\unit\\easypoi\\annotation\\an_bar6.jpg";
        // 构建条形码
        BannerUtil.generateFile(billNo, fileUrl);

        // 按字符串形式设置图片，会从指定位置读取文件
        student1.setBarCode(fileUrl);

        StudentEntity student2 = new StudentEntity();
        student2.setId("102");
        student2.setName("李四ddd");
        student2.setSex(16);
        student2.setBirthday(new Date());
        student2.setRegistrationDate(new Date());
        student2.setBarCode(fileUrl);
        ...
        cell.setCellValue("张三ttt"); //是引用传递的


        Row row1 = ((HSSFSheet)sheet).createRow(7); //Excel中新增一列（文本格式）
        Cell cell1 = row1.createCell(2);
        cell1.setCellValue("哈哈哈tt");

        // 将数据写到数据流中
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        workbook.write(bos);

        String fileName = "D:\\self_project\\practice\\practice-gz\\src\\main\\java\\basic\\unit\\easypoi\\annotation\\annotation7.xls";
